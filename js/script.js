const global = {
  ...
  api: {
    apiKey: 'fb53f0572e86d48cf618118b61aab590',
...
async function displayShowDetails() {
  const serial_id = window.location.search.split('=')[1];
  const show = await fetchAPIData(`tv/${serial_id}`);
...
async function search() {
  ...
  if (global.search.term !== '' && global.search.term !== null) {
    ...
    if (results.length === 0) {
      showAlert('No result term', 'alert-success');
      return;
    }
